...
class Categories extends BackendBaseActionIndex
{
	...
	private function loadDataGrid()
	{
		...
		$this->multipleCategoriesAllowed = BackendModel::getModuleSetting('agenda', 'allow_multiple_categories', true);
	...
	protected function parse()
	{
		...
		else $this->tpl->assign('showAgendaAddCategory', false);
	}
	...
	public static function setClickableCount($count, $link)
	{
		...
		if($count > 1) return '<a href="' . $link . '">' . $count . ' ' . BL::getLabel('Agenda') . '</a>';
		if($count == 1) return '<a href="' . $link . '">' . $count . ' ' . BL::getLabel('Agenda') . '</a>';
		else return BL::getLabel('NoAgenda');
	}
}
